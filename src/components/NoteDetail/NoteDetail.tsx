...
import DeleteNoteModal, {
  type DeleteNoteModalRef,
} from "./../DeleteNoteModal/DeleteNoteModal";
import ArchiveNoteModal, {
  type ArchiveNoteModalRef,
} from "./../ArchiveNoteModal/ArchiveNoteModal";
...
export default function NoteDetail() {
  ...
  const notes = isArchivedRoute
    ? archivedNotes
    : isTagRoute && tagId
    ? getNotesByTag(tagId)
    : activeNotes;
  const [note, setNote] = useState<Note | undefined>(notes.find((n) => n.id === noteId));
  ...
  function openDialog(
    ref: React.RefObject<DeleteNoteModalRef | ArchiveNoteModalRef | null>
  ) {
    ref.current?.open();
  }
        ...
        <form className={styles["note"]} onSubmit={handleSubmit}>
          ...
          <div className={styles["note__properties"]}>
            <NoteDetailProperty
              icon={<TagIcon />}
              label="Tags"
              value={<input id="tags" name="tags" placeholder="Add tags separated by commas (e.g. Work, Planning)" value={note.tags.map((t) => capitalize(t)).join(", ")} onChange={(e) => setNote({...note, tags: formatTags(e.target.value)})}/>}
            />
            {isArchivedRoute && (
              <NoteDetailProperty
                icon={<StatusIcon />}
        ...
        <aside className={styles["note__controls-desktop"]}>
          {!isArchivedRoute && (
            <button
              ...
              onClick={() => openDialog(archiveNoteDialog)}
            >
              <ArchiveIcon />
          ...
          <button
            className="btn btn--border"
            onClick={() => openDialog(deleteNoteDialog)}
          >
            <DeleteIcon />
